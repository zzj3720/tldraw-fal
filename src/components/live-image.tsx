...
import {redraw} from "@/components/prompt-input";
...
export function LiveImage() {
    ...
    const prePrompt = useRef<string | null>(null);
    ...
    const onDrawingChange = useCallback(
        debounce(async () => {
            ...
            if (shapesDigest === imageDigest.current && prePrompt.current === prompt) {
                return;
            }
            imageDigest.current = shapesDigest;
            prePrompt.current = prompt;
    ...
    return (
        <div
            className="flex flex-row w-[1060px] h-[560px] absolute bg-indigo-200 border border-indigo-500 rounded space-x-4 p-4 pb-8">
            <div className="flex-1 h-[512px] bg-white border border-indigo-500">
                <div className="flex flex-row items-center px-4 py-2 space-x-2">
                    <span className="font-mono text-indigo-900/50">draw in this box</span>
                </div>
            </div>
            <div className="flex-1 h-[512px] bg-white border border-indigo-500">
...
type LiveImageShape = TLBaseShape<"live-image", {
    w: number;
    h: number
}>;
